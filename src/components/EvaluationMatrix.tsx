...
export const EvaluationMatrix: React.FC<EvaluationMatrixProps> = ({ alternatives, criteria, evaluations }) => {
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
        <div className="max-w-md mx-auto">
          <h3 className="text-lg font-medium text-slate-900 mb-2">Матриця оцінювання</h3>
          <p className="text-slate-500">
            Будь ласка, додайте хоча б одну **Альтернативу** та один **Критерій**, щоб почати оцінювання.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-4 border-b border-slate-100 bg-slate-50/50">
        <h2 className="text-lg font-semibold text-slate-800">Матриця оцінювання</h2>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-50">
              ...
              {criteria.map(crit => (
                <th key={crit.id} className="p-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-slate-100 min-w-[120px]">
                  <div className="flex flex-col items-center gap-1">
                    <span>{crit.name}</span>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-full ${crit.type === 'maximize' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
            ...
            {alternatives.map(alt => (
              <tr key={alt.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-4 border-r border-slate-100">
                  ...
                  {alt.description && <div className="text-xs text-slate-400 truncate max-w-[180px]">{alt.description}</div>}
                </td>
                {criteria.map(crit => {
                  ...
                  return (
                    <td key={crit.id} className="p-2 text-center">
                      <input
                        ...
                        onChange={(e) => handleValueChange(alt.id, crit.id, e.target.value)}
                        placeholder="-"
                      />
                    </td>
                  );
