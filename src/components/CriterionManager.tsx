...
export const CriterionManager: React.FC<CriterionManagerProps> = ({ criteria }) => {
  ...
  const handleUpdate = async (id: string) => {
    if (!name.trim()) return;
    const finalWeight = isNaN(weight) ? 1 : weight;
    await decisionService.updateCriterion(id, name, type, finalWeight, description);
    setEditingId(null);
    setName('');
    setType('maximize');
    setWeight(1);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="divide-y divide-slate-100">
          ...
          <div className="p-4 bg-emerald-50/30 animate-in fade-in slide-in-from-top-2">
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-500 uppercase">Вага (0.1 - 1.0)</label>
                <input
                  type="number"
                  min="0.1"
                  max="1"
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={handleAdd} className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium">Зберегти</button>
              </div>
            </div>
          </div>
        )}
        ...
        {criteria.length === 0 && !isAdding && (
          <div className="p-8 text-center text-slate-400 italic">Критеріїв поки немає.</div>
        )}

        {criteria.map((crit) => (
          <div key={crit.id} className="p-4 hover:bg-slate-50 transition-colors group">
            {editingId === crit.id ? (
              <div className="space-y-3">
                <input
                  type="text"
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button onClick={() => handleUpdate(crit.id)} className="p-1.5 text-emerald-500 hover:text-emerald-600"><Check size={20} /></button>
                </div>
              </div>
            ) : (
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-full ${crit.type === 'maximize' ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                      {crit.type === 'maximize' ? 'максимізація' : 'мінімізація'}
                    </span>
                    <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-600">
                      вага: {crit.weight || 1}
                    </span>
                  </div>
                  <p className="text-sm text-slate-500 mt-1">{crit.description || 'Опис відсутній.'}</p>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(crit)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-md"><Edit2 size={16} /></button>
                  <button onClick={() => decisionService.deleteCriterion(crit.id)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md"><Trash2 size={16} /></button>
                </div>
              </div>
            )}
          </div>
